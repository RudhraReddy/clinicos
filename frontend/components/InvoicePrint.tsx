            ...
            {billItems.map((item, idx) => {
                ...
                return (
                    <div key={idx}>
                        ...
                        {/* Only present fields are rendered — a missing field (no
                            manufacturer, no HSN, etc.) is simply absent from the row
                            rather than leaving an empty reserved column of blank
                            space. This means the two rows won't always column-align
                            with each other when they have different fields present,
                            but that's the trade-off for never showing dead space. */}
                        {detailLine1.length > 0 && (
                            <div style={{ display: "flex", flexWrap: "wrap", columnGap: "14px", fontSize: "7.5pt", wordBreak: "keep-all", overflowWrap: "normal" }}>
